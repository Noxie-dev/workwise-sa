...
router.get('/overview', async (req, res, next) => {
  try {
    ...
    const views = interactionRows.filter(
      interaction => interaction.interactionType === 'view'
    ).length;
    const interviews = applicationRows.filter(
    ...
    res.json(
      adminAnalyticsSchema.parse({
        ...
        userActivity: {
          daily: [7, 6, 5, 4, 3, 2, 1].map(divisor =>
            bucketCount(activeUsers || userRows.length, divisor)
          ),
        ...
        conversionRates: {
          viewToApplication:
            views > 0 ? Number(((applicationRows.length / views) * 100).toFixed(1)) : 0,
          applicationToInterview:
            applicationRows.length > 0
              ? Number(((interviews / applicationRows.length) * 100).toFixed(1))
              : 0,
