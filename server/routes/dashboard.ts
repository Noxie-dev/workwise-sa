...
function inferMatchScore(title: string, userSkills: string[]) {
  ...
  const overlap = userSkills.filter(skill =>
    titleWords.some(
      word => word.includes(skill.toLowerCase()) || skill.toLowerCase().includes(word)
    )
  ).length;
...
router.get('/job-distribution', async (req, res, next) => {
  try {
    ...
    jobRows.forEach(job => {
      ...
      trendCounts.set(
        toIsoMonth(job.createdAt),
        (trendCounts.get(toIsoMonth(job.createdAt)) || 0) + 1
      );
    });

    res.json(
      paginatedResponseSchema(jobDistributionPayloadSchema).parse({
        data: {
...
router.get('/job-recommendations', async (req, res, next) => {
  try {
    ...
    res.json(
      paginatedResponseSchema(z.array(jobRecommendationSchema)).parse({
...
router.get('/skills-analysis', async (req, res, next) => {
  try {
    ...
    const marketDemand = keywords
      .map(skill => ({
        skill: skill.replace(/\b\w/g, char => char.toUpperCase()),
        demand: jobRows.filter(job =>
          `${job.title} ${job.description}`.toLowerCase().includes(skill)
        ).length,
        growth: Math.min(
          25,
          5 +
            jobRows.filter(job => `${job.title} ${job.description}`.toLowerCase().includes(skill))
              .length *
              3
        ),
      }))
    ...
    const recommendations = marketDemand
      .filter(
        item => !userSkills.some(skill => skill.skill.toLowerCase() === item.skill.toLowerCase())
      )
      .slice(0, 3)
      .map(item => ({
        skill: item.skill,
    ...
    res.json(
      paginatedResponseSchema(skillsAnalysisPayloadSchema).parse({
        data: {
