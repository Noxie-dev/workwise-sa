...
describe('AI CV routes', () => {
  ...
  it('reserves usage, generates structured CV content, and completes the usage event', async () => {
    ...
    expect(mockedMeter.reserveUsage).toHaveBeenCalledWith(
    ...
    expect(mockedMeter.completeUsage).toHaveBeenCalledWith(
      expect.objectContaining({
        usageEventId: 101,
      })
    );
  ...
  it('returns forbidden when entitlement quota is exhausted', async () => {
    mockedMeter.reserveUsage.mockRejectedValue(
      Object.assign(new Error('AI generation quota exhausted or feature disabled'), {
        statusCode: 403,
      })
    );
