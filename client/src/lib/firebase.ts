...
const resolvedMessagingSenderId =
  hasValue(rawFirebaseEnv.messagingSenderId) && !isPlaceholderValue(rawFirebaseEnv.messagingSenderId)
...
export const firebaseStatus = {
  ...
  mode: firebaseClientOpsEnabled ? (configWarnings.length > 0 ? "emulator-placeholder" : "configured") : "disabled",
...
if (usingDevFallbackConfig.length > 0) {
  console.warn(
    "Using local Firebase placeholder defaults for:",
    usingDevFallbackConfig.join(", "),
  );
}
...
const actionCodeSettings: ActionCodeSettings = {
  url: import.meta.env.VITE_AUTH_EMAIL_LINK_SIGN_IN_URL || `${fallbackOrigin}/auth/email-signin-complete`,
