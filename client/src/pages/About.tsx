...
const About = () => {
  ...
  const missionData = {
    title: 'Our Mission',
    content:
      'WorkWise SA is dedicated to bridging the gap between talented entry-level job seekers and forward-thinking employers across South Africa. We believe that everyone deserves access to meaningful career opportunities, regardless of their background or experience level.',
  ...
  const whatWeDoData = {
    ...
    jobSeekers: [
      {
        text: 'Comprehensive job search platform with advanced filtering',
        icon: <Search size={24} className="text-accent" />,
      },
      {
        text: 'Professional CV builder with industry-specific templates',
        icon: <Briefcase size={24} className="text-accent" />,
      },
      {
        text: 'WiseUp learning platform with career development resources',
        icon: <BookOpen size={24} className="text-accent" />,
      },
      {
        text: 'Personalized job matching based on skills and preferences',
        icon: <UserCheck size={24} className="text-accent" />,
      },
      {
        text: 'Interview preparation and career guidance tools',
        icon: <TrendingUp size={24} className="text-accent" />,
      },
    ],
    employers: [
      {
        text: 'Access to a diverse pool of entry-level talent',
        icon: <Users size={24} className="text-accent-foreground" />,
      },
      {
        text: 'Advanced candidate filtering and matching',
        icon: <Filter size={24} className="text-accent-foreground" />,
      },
      {
        text: 'Efficient recruitment tools and applicant tracking',
        icon: <Zap size={24} className="text-accent-foreground" />,
      },
      {
        text: 'Employer branding opportunities',
        icon: <Star size={24} className="text-accent-foreground" />,
      },
      {
        text: 'Recruitment analytics and insights',
        icon: <BarChart3 size={24} className="text-accent-foreground" />,
      },
    ],
  };
  ...
  const whyChooseUsData = {
    ...
    reasons: [
      {
        title: 'Focus on Entry-Level Talent',
        description:
          'We specialize in connecting emerging professionals with their first career opportunities.',
        icon: <UserCheck size={32} className="text-primary" />,
      },
      {
        title: 'Local Expertise',
        description: 'Deep understanding of the South African job market and employment landscape.',
        icon: <MapPin size={32} className="text-primary" />,
      },
      {
        title: 'Comprehensive Resources',
        description: 'Beyond job matching, we provide tools for professional development.',
        icon: <BookOpen size={32} className="text-primary" />,
      },
      {
        title: 'Technology-Driven',
        description: 'Modern platform built with React, ensuring smooth user experience.',
        icon: <Cpu size={32} className="text-primary" />,
      },
      {
        title: 'Privacy-First',
        description: 'Robust data protection and user privacy measures.',
  ...
  const valuesData = {
    ...
    values: [
      ...
      {
        name: 'Excellence',
        description: 'Continuous improvement and quality service delivery.',
      ...
      {
        name: 'Integrity',
        description: 'Ethical practices and responsible business conduct.',
        icon: <ShieldQuestion size={32} className="text-primary" />,
      },
    ],
  };
  ...
  const contactData = {
    ...
    contacts: [
      ...
      },
      {
        ...
        email: 'partnerships@workwisesa.co.za',
  ...
  return (
    <>
      <CustomHelmet
        ...
        description="Learn about WorkWise SA's mission to connect young South Africans with entry-level jobs and career opportunities."
      />
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-12">
              <Card className="transform transition-all duration-300 hover:scale-105 hover:shadow-xl bg-accent/10">
                <CardHeader>
                  <CardTitle className="text-3xl text-accent-foreground/90">
                    For Job Seekers
                  </CardTitle>
                </CardHeader>
          ...
          >
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {whyChooseUsData.reasons.map((reason, index) => (
                <Card
                  key={index}
                  className="text-center transform transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl bg-card/90 backdrop-blur-sm"
                >
                  <CardHeader>
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
              {valuesData.values.map((value, index) => (
                <Card
                  key={index}
                  className="text-center p-6 transform transition-all duration-300 hover:shadow-lg hover:bg-primary/5 group"
                >
                  ...
                  <h3 className="text-xl font-medium mb-1 text-primary/90 group-hover:text-accent-foreground">
                    {value.name}
                  </h3>
          ...
          >
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {contactData.contacts.map((contact, index) => (
                <Card
                  key={index}
                  className="text-center transform transition-all duration-300 hover:scale-105 bg-card/90 backdrop-blur-sm"
                >
                  <CardHeader>
                  ...
                  <CardContent>
                    <a
                      ...
                      className="text-accent hover:text-accent-foreground/80 transition-colors"
                    >
                      {contact.email}
                    </a>
                  </CardContent>
