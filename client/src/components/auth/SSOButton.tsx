...
import { SSOButtonProps } from '@/types/auth';

/**
 * SSO Button Component
 * Handles social sign-on authentication
 */
const SSOButton: React.FC<SSOButtonProps> = ({
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      console.error(`${provider.name} sign in error:`, error);
      onError(error.message || `Failed to sign in with ${provider.name}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getButtonStyle = () => {
    switch (provider.id) {
      ...
      case 'facebook':
        return 'hover:bg-blue-50 border-blue-300 text-blue-600';
      case 'microsoft':
        return 'hover:bg-blue-50 border-blue-300 text-blue-600';
      case 'apple':
  ...
  return (
    <Button
      ...
      className={`w-full ${getButtonStyle()} ${className}`}
    >
      {isLoading ? (
        <>
