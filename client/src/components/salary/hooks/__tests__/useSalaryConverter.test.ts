...
describe('useSalaryConverter', () => {
  it('converts monthly salary to other frequencies correctly', () => {
    const { result } = renderHook(() => useSalaryConverter(50000, 'monthly', 8, 5, 52));
  ...
  it('converts annual salary to other frequencies correctly', () => {
    const { result } = renderHook(() => useSalaryConverter(600000, 'annual', 8, 5, 52));
  ...
  it('converts hourly rate to other frequencies correctly', () => {
    const { result } = renderHook(() => useSalaryConverter(300, 'hourly', 8, 5, 52));
  ...
  it('handles zero and invalid inputs gracefully', () => {
    const { result } = renderHook(() => useSalaryConverter(0, 'monthly', 8, 5, 52));
  ...
  it('uses advanced settings when provided', () => {
    ...
    const { result } = renderHook(() => useSalaryConverter(50000, 'monthly', 6, 4, 48));
