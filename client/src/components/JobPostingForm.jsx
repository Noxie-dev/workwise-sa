...
const JobPostingForm = () => {
  ...
  ); // Disable autosave during/after submission
    ...
    if (savedDraft && Object.keys(savedDraft).length > 0) {
      const isDifferentFromInitial = Object.keys(INITIAL_FORM_STATE).some(key => {
        if (key === 'logo' && savedDraft[key]) {
          // Special handling for logo if it's complex
    ...
    () => [
      {
        ...
        props: { onInitiateAIAssist: handleAIAssist },
      },
      {
        name: 'Description',
      ...
      },
      {
        ...
        props: { onEditStep: stepIndex => setCurrentStep(stepIndex), categories: categories },
      },
    ],
    [categories]
  ); // handleAIAssist is stable due to its definition
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-8 text-center">
        <Card className="max-w-md w-full">
          <CardContent className="p-8">
            ...
            {submissionResponse?.jobId && (
              <p className="text-gray-600 mb-1">
                Your job post ID: "{submissionResponse.jobId}" has been successfully submitted.
              </p>
            )}
            {submissionResponse?.message && (
              <p className="text-gray-600 mb-6">{submissionResponse.message}</p>
            )}
            <Button
              onClick={() => {
                formState.reset();
                setCurrentStep(0); /* Potentially navigate or reset mutation state */
              }}
            >
              Post Another Job
            </Button>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8">
      {showDraftDialog && draftDataToLoad && (
        <AlertDialog open={showDraftDialog} onOpenChange={setShowDraftDialog}>
          <AlertDialogContent>
            ...
            <AlertDialogDescription>
              We found an unsaved draft from your last session. Would you like to continue editing
              it or start fresh?
            </AlertDialogDescription>
            <AlertDialogFooter>
              <AlertDialogCancel onClick={discardDraftAndReset}>
      ...
      <form
        ...
        className="max-w-4xl mx-auto bg-white p-6 sm:p-8 rounded-xl shadow-2xl"
      >
        <div className="mb-8">
        ...
        <div className="mb-8 overflow-x-auto">
          <nav className="flex border-b border-gray-200 -mx-3 sm:-mx-0">
            {steps.map((step, index) => (
              ...
              >
                <span
                  className={`mr-2 h-5 w-5 ${currentStep === index ? 'text-blue-600' : 'text-gray-400'}`}
                >
                  {step.icon}
                </span>
                {step.name}
        ...
        <div className="mt-10 pt-6 border-t flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
            {submissionError && (
              <p className="text-sm text-red-600 order-first sm:order-none w-full text-center sm:text-left">
            ...
            <Button
              ...
              className="text-xs text-gray-500 hover:text-red-600 flex items-center gap-1 w-full sm:w-auto justify-center"
            >
