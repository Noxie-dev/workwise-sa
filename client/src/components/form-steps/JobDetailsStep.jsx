...
const JobDetailsStep = ({ formState, onInitiateAIAssist }) => {
  ...
  const groupedCategories = useMemo(() => {
    ...
    return categories.reduce((acc, category) => {
      const groupName =
        category.group.charAt(0).toUpperCase() + category.group.slice(1).replace('_', ' ');
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <FormField
          ...
          required
          hint="Be specific, e.g., 'Senior React Developer'"
        >
            ...
            <Dialog open={isAiDialogOpen} onOpenChange={setIsAiDialogOpen}>
              <DialogTrigger asChild>
                <Button
                  ...
                  type="button"
                  disabled={!canShowAIDialog}
                  title={
                    !canShowAIDialog
                      ? 'Enter title & category first'
                      : 'AI Job Description Assistant'
                  }
                  onClick={() => {
                    if (canShowAIDialog) setIsAiDialogOpen(true);
                  }}
                >
              ...
              <DialogContent className="sm:max-w-lg">
                ...
                <DialogDescription>
                  Let AI help craft a job description. Choose a style or provide custom
                  instructions.
                </DialogDescription>
                <div className="grid gap-4 py-4">
                  <p>
                    Using: <strong>{values.title}</strong> in{' '}
                    <strong>{selectedCategoryName || values.category}</strong>
                  </p>
                  <Tabs
                    defaultValue={values.aiDescriptionType || 'basic'}
                    onValueChange={val => handleChange('aiDescriptionType', val)}
                    className="w-full"
                  >
                    ...
                    <TabsContent value="basic" className="p-4 border rounded-md mt-2 text-sm">
                      Generate a concise job description covering essential duties and requirements.
                    </TabsContent>
                    <TabsContent value="detailed" className="p-4 border rounded-md mt-2 text-sm">
                      Generate a comprehensive description with detailed responsibilities,
                      qualifications, and benefits.
                    </TabsContent>
                    <TabsContent value="custom" className="p-4 border rounded-md mt-2">
                      <Textarea
                        placeholder="e.g., Emphasize teamwork and a fast-paced environment. Mention our upcoming Project X."
                        value={values.aiCustomPrompt || ''}
                        onChange={e => handleChange('aiCustomPrompt', e.target.value)}
        ...
        >
          <Select
            ...
            onOpenChange={open => {
              if (!open && !values.category) handleBlur('category');
            }}
          >
            ...
            <SelectContent>
              ...
              {Object.entries(groupedCategories).map(([groupName, cats]) => (
                <React.Fragment key={groupName}>
                  <div className="px-3 py-1.5 text-xs font-semibold text-gray-500 uppercase sticky top-0 bg-white">
                    {groupName}
                  </div>
                  {cats.map(cat => (
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        >
          <Select
            ...
            onOpenChange={open => {
              if (!open && !values.jobType) handleBlur('jobType');
            }}
          >
            ...
            <SelectContent>
              {['full-time', 'part-time', 'contract', 'temporary', 'internship', 'volunteer'].map(
                type => (
                ...
                <ul className="absolute z-20 mt-1 w-full rounded-md border bg-white shadow-lg max-h-60 overflow-auto">
                  {locationSuggestions.map(suggestion => (
                    <li
                      ...
                      onMouseDown={() => {
                        // Use onMouseDown to fire before blur on input
              ...
              <Checkbox
                ...
                onCheckedChange={checked => {
                  ...
                  if (checked) {
                    ...
                    if (errors.location) setLocationSuggestions([]);
                    handleBlur('location');
                  }
                }}
              />
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <FormField
          id="salary-range"
          label="Salary Range (Optional, ZAR)"
          error={(touched.salaryMin && errors.salaryMin) || (touched.salaryMax && errors.salaryMax)}
          hint="Increases application rates"
        >
          <div className="space-y-2">
            ...
            {(errors.salaryMin && !errors.salaryMax && (
              ...
              (errors.salaryMin && errors.salaryMax && errors.salaryMin === errors.salaryMax && (
                <p className="text-xs text-red-500 mt-1">{errors.salaryMin}</p>
              )) ||
              (errors.salaryMin && errors.salaryMax && errors.salaryMin !== errors.salaryMax && (
                <>
