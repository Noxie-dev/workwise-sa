...
const CompanyInfoStep = ({ formState, onInitiateAIAssist }) => {
  ...
  useEffect(() => {
    if (values.logo && typeof values.logo === 'object' && values.logo instanceof File) {
      ...
      // Could be a data URL from a draft
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <FormField id="company-size" label="Company Size (Optional)">
          <Select
            ...
            onOpenChange={open => {
              if (!open) handleBlur('companySize');
            }}
          >
        ...
        <FormField
          id="company-logo"
          label="Company Logo (Optional, max 2MB)"
          hint="Recommended: Square, PNG/JPG"
        >
      ...
      <FormField
        ...
        hint="Briefly describe your company. Markdown supported."
      >
        <div className="space-y-2">
          <div className="flex justify-end">
            <Button
              ...
              onClick={() =>
                onInitiateAIAssist(
                  'companyBio',
                  { companyName: values.companyName, category: values.category },
                  {}
                )
              }
              disabled={!canUseAIForBio}
              type="button"
              title={!canUseAIForBio ? 'Enter company name first' : 'Generate with AI'}
            >
          ...
          <Textarea
            ...
            placeholder="e.g., We are a leading tech company focused on innovation..."
          />
        </div>
      </FormField>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <FormField
          ...
          required
          hint="Applications will be sent here."
        >
          <Input
            id="contact-email"
            type="email"
