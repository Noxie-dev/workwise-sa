...
export const LoadingScreenExample = () => {
  ...
  const [loadingVariant, setLoadingVariant] = useState<
    'default' | 'custom-message' | 'inline' | 'large'
  >('default');
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-wrap gap-4">
        <Button onClick={() => simulateLoading('default')}>Default Loading Screen</Button>

        <Button onClick={() => simulateLoading('custom-message')}>Custom Message</Button>

        <Button onClick={() => simulateLoading('inline')}>Inline Loading</Button>

        <Button onClick={() => simulateLoading('large')}>Large Spinner</Button>
      </div>
