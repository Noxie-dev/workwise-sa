...
const useJobFormState = initialState => {
    ...
    const validateField = (field, value, currentValues) => {
      ...
      switch (field) {
        ...
        case 'jobType':
          ...
          break;
        case 'location':
          if (!value?.trim() && !currentValues.isRemote)
            newErrors.location = 'Location is required unless remote';
        ...
        case 'companyWebsite':
          if (value?.trim() && !/^https?:\/\/[^\s/$.?#].[^\s]*$/i.test(value))
            newErrors.companyWebsite = 'Invalid website URL (e.g., http://example.com)';
  ...
  const validateAll = useCallback(() => {
    ...
    requiredFields.forEach(field => {
      ...
      switch (field) {
        case 'title':
          if (!values.title?.trim()) allNewErrors.title = 'Job title is required';
          break;
        case 'category':
          if (!values.category) allNewErrors.category = 'Category is required';
          break;
        case 'jobType':
          if (!values.jobType) allNewErrors.jobType = 'Job type is required';
          break;
        case 'description':
          if (!values.description?.trim()) allNewErrors.description = 'Job description is required';
          break;
        case 'companyName':
          if (!values.companyName?.trim()) allNewErrors.companyName = 'Company name is required';
          break;
        case 'contactName':
          if (!values.contactName?.trim()) allNewErrors.contactName = 'Contact name is required';
          break;
        case 'contactEmail':
          if (!values.contactEmail?.trim()) allNewErrors.contactEmail = 'Contact email is required';
          else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(values.contactEmail))
            allNewErrors.contactEmail = 'Invalid email address';
          break;
        default:
          break;
      }
    ...
    if (
      ...
      !/^https?:\/\/[^\s/$.?#].[^\s]*$/i.test(values.companyWebsite)
    ) {
