...
import { Permission, UserRole, UserUpdate, RegisterData, AuthResult } from '@shared/auth-types';
...
export const useAuth = () => {
  ...
  const registerWithToast = useCallback(
  ...
  const resetPasswordWithToast = useCallback(
    async (email: string) => {
  ...
  const updateProfileWithToast = useCallback(
...
export const useAdminAuth = () => {
  ...
  const canManageUsers = useMemo(() => auth.hasPermission('admin:users'), [auth.hasPermission]);

  const canManageSettings = useMemo(
    () => auth.hasPermission('admin:settings'),
    [auth.hasPermission]
  );

  const canViewAnalytics = useMemo(
    () => auth.hasPermission('admin:analytics'),
    [auth.hasPermission]
  );

  const canManageContent = useMemo(() => auth.hasPermission('admin:content'), [auth.hasPermission]);

  const canManageSecurity = useMemo(
    () => auth.hasPermission('admin:security'),
    [auth.hasPermission]
  );
...
export const useEmployerAuth = () => {
  ...
  const canCreateJobs = useMemo(() => auth.hasPermission('jobs:create'), [auth.hasPermission]);

  const canEditJobs = useMemo(() => auth.hasPermission('jobs:edit'), [auth.hasPermission]);

  const canDeleteJobs = useMemo(() => auth.hasPermission('jobs:delete'), [auth.hasPermission]);

  const canSendNotifications = useMemo(
    () => auth.hasPermission('notifications:send'),
    [auth.hasPermission]
  );
...
export const useProfileAuth = () => {
  ...
  const canEditProfile = useMemo(() => auth.hasPermission('profile:edit'), [auth.hasPermission]);

  const canViewProfile = useMemo(() => auth.hasPermission('profile:view'), [auth.hasPermission]);

  const needsProfileCompletion = useMemo(() => !auth.isProfileComplete, [auth.isProfileComplete]);
...
export const useJobAuth = () => {
  ...
  const canViewJobs = useMemo(() => auth.hasPermission('jobs:view'), [auth.hasPermission]);

  const canApplyToJobs = useMemo(() => auth.hasPermission('jobs:apply'), [auth.hasPermission]);
