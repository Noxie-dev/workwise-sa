...
export const calculateHiringScore = (company: CompanyHiringMetrics): TopHiringCompanyResult => {
  ...
  const urgencyScore = company.urgentPositions
    ? Math.min(company.urgentPositions / 10, 1) * SCORING_WEIGHTS.URGENCY
    : 0;
...
export const getTopHiringCompanies = (
  ...
  return sortedCompanies.slice(0, limit).map((company, index) => ({
    ...company,
    rank: index + 1,
  }));
};
 ...
 */
export const fetchTopHiringCompanies = async (
  limit: number = 3
): Promise<TopHiringCompanyResult[]> => {
  try {
