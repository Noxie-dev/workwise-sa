...
if (fs.existsSync('dist/public')) {
  ...
  if (fs.existsSync('dist/public/index.html')) {
    ...
    warning('Build output missing index.html - run pnpm run build');
  }
} else {
  warning('Build output directory missing - run pnpm run build');
}
...
if (hasErrors) {
  ...
  console.log('\n🚀 Next steps:');
  console.log('   pnpm run deploy:firebase     # Deploy hosting');
  console.log('   pnpm run deploy:firebase:all # Deploy everything');
  process.exit(0);
}
